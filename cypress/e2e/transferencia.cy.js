describe('Transferências', () => {
    ...
    it('Deve transferir quando informo dados e valor válidos', () => {
        ...
        cy.get('@campo-conta-origem').contains('João da Silva com saldo de R$ 17461.00 (Ativa)').click()

        cy.get('label[for="conta-destino"]').parent().as('campo-conta-destino')
        cy.get('@campo-conta-destino').click()
        cy.get('@campo-conta-destino').contains('Maria Oliveira com saldo de R$ 20539.00 (Ativa)').click() 

        cy.get(':nth-child(4) > label').click().type('11')
